...
export default function ThemeTiles({ themes }) {
  const pick = (key) => {
    ...
    document.querySelector("#archive")?.scrollIntoView({ behavior: reduced ? "auto" : "smooth", block: "start" });
  };
  return (
      ...
      {themes.map((t, i) => (
        <Reveal key={t.key} y={14} delay={0.04 * i} className={`h-full ${i === 0 ? "sm:col-span-2" : ""}`}>
          <SpotlightCard
            ...
            type="button"
            onClick={() => pick(t.key)}
            className="flex h-full w-full flex-col rounded-2xl border border-line p-6 text-left transition-[border-color,transform] duration-300 hover:border-white/40 active:scale-[0.99]"
          >
            ...
            <span className="mt-auto pt-5 inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.12em] text-dim group-hover:text-white transition-colors">
              Filter the archive
